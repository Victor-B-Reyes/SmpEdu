...
export default class PageSetup implements OnInit {
  ...
  protected readonly visibleTabs = computed(() => {
    return this.tabs().filter(tab => 
      this.authService.hasDetailedPermission('Setup', tab.label)
    );
  });
  ...
  private syncActiveTab(): void {
    ...
    const currentTab = this.tabs().find(tab => url.includes(`/setup/${tab.route}`));
    if (currentTab) {
  ...
  selectTab(tabId: string): void {
    ...
    if (tab) {
      ...
      // Navega a la subruta (ej. /setup/user)
      this.router.navigate(['/setup', tab.route]);
    }
  }
  ...
  private navigateTo(route: string): void {
    ...
    this.router.navigate([`/setup/${route}`]).catch((err) => {
